angular.module('itunes').controller('mainCtrl', function($scope, itunesService){
  ...
  //Alternatively you could just rename what it's looking for to what the object returns and it will post without the for loop. Maybe?
  $scope.gridOptions = {
      data: 'songData',
      height: '110px',
  ...
  //Also note that that method should be retuning a promise, so you could use .then in this function.
    ...
    $scope.getSongData = function() {
      ...
      itunesService.getArtist($scope.artist).then(function(dataFromService) {
        $scope.artist = '';
        var newArray = [];
        function Item(albumArt, artist, album, collectionPrice, play, type, songTitle){
        ...
        for (var i = 0; i < dataFromService.length; i++) {
          var item = new Item(dataFromService[i].artworkUrl60,dataFromService[i].artistName,dataFromService[i].collectionCensoredName,dataFromService[i].collectionPrice,dataFromService[i].previewUrl,dataFromService[i].kind,dataFromService[i].trackCensoredName);
          newArray.push(item);
          }
  ...
  //If everything worked you should see a huge array of objects inside your console. That's great! But unfortunately that's
  //not what ng-grid is expecting. What you need to do now
  ...
  */
  ...
  //iTunes data pushing into your new array objects that look like the above data.
    ...
    //Code here
    /*var newArray = [];
    function Item(album, artist, collection, collectionPrice, play, type){
      this.AlbumArt = albumArt,
      this.Artist = artist,
    ...
    for (var prop in $scope.songData) {
      var item = new Item(prop.AlbumArt,prop.Artist,prop.Collection,prop.CollectionPrice,prop.Play,prop.Type);
      newArray.push(item);
      }
    console.log(newArray);*/
